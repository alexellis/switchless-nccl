...
The in-process verifier reads `/proc/PID/maps`, resolves symlinks, and uses
`dladdr` to prove that the process-global `ncclGetVersion` symbol resolves to
the expected file. Modern PyTorch may also map its wheel-bundled NCCL as an
implementation detail; the additional mapping is reported but is not itself
failure when the preloaded candidate wins symbol resolution. External PID
inspection cannot call `dladdr` in the target, so that mode conservatively
requires exactly one mapped NCCL identity.
...
The final live gate must observe the `SWITCHLESS/HARDENED` listener, Tree, and
PAT diagnostics with `NCCL_DEBUG=INFO`, confirm Ring
